...
function LibraryGalleryItem(props: Props) {
  const { _id, title, path, width, height, alt, size } = props.image;

  const small = props.variant && props.variant === 'small';

  return (
    <Link
      href={`/admin/library/edit/${_id}`}
          ...
          <div className="px-4 pb-4 flex flex-col justify-center items-center">
            ...
            <div className="flex flex-row space-x-8">
              <Link
                ...
                href={`library/edit/${_id}`}
              >
                Edit
              </Link>
              <Link
                className="hover:text-palette-brown"
                href={`library/delete/${_id}`}
              >
                Delete
              </Link>
            </div>
...
export default LibraryGalleryItem;
