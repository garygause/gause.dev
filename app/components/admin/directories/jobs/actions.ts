...
export async function saveJobForm(id: string, formData: FormData) {
  ...
  try {
    if (id) {
      const { data, meta } = await client.directories.admin.updateJob(id, {
        companyId: companyId,
        title: title,
        url: url,
      ...
      const { data, meta } = await client.directories.admin.createJob({
        companyId: companyId,
        title: title,
        url: url,
        companyUrl: companyUrl,
  ...
  revalidatePath(PATHS.directoriesJobsEdit, 'page');
