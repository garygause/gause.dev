...
export default async function PeopleTable({ people }: { people: Person[] }) {
  return (
    <>
              ...
              {people?.map((person) => (
                ...
                >
                  <div className="flex w-full items-center justify-between pt-4">
                    <div>
                      <p className="text-xl font-medium">{person?.firstName}</p>
                    </div>
                    <div>
                      <p className="text-xl font-medium">{person?.lastName}</p>
                    </div>
                    <div>
                      <p className="text-xl font-medium">{person?.slug}</p>
                    </div>
                    <div className="flex justify-end gap-2">
                      <UpdateButton
                        url={`${PATHS.directoriesPeopleEdit}/${person.id}`}
                      />
                      <DeleteButton
                        url={`${PATHS.directoriesPeopleDelete}/${person.id}`}
                      />
                    </div>
                  </div>
                </div>
