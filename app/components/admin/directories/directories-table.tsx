...
export default async function DirectoriesTable({
  directories,
}: {
              ...
              {directories?.map((directory) => (
                ...
                >
                  <div className="flex w-full items-center justify-between pt-4">
                    <div>
                      <p className="text-xl font-medium">{directory?.name}</p>
                    </div>
                    <div className="flex justify-end gap-2">
                      <UpdateButton
                        url={`${PATHS.directoriesEdit}/${directory.id}`}
                      />
                      <DeleteButton
                        url={`${PATHS.directoriesDelete}/${directory.id}`}
                      />
                    </div>
                  </div>
                </div>
