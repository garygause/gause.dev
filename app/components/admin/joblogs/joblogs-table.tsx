...
export default async function JobLogsTable({ joblogs }: { joblogs: JobLog[] }) {
  return (
    <>
              ...
              {joblogs?.map((joblog) => (
                ...
                >
                  <div className="flex w-full items-center justify-between pt-4">
                    <div>
                      <p className="text-xl font-medium">{joblog?.name}</p>
                    </div>
                    <div className="flex justify-end gap-2">
                      <UpdateButton url={`${PATHS.joblogsEdit}/${joblog.id}`} />
                      <DeleteButton
                        url={`${PATHS.joblogsDelete}/${joblog.id}`}
                      />
                    </div>
                  </div>
                </div>
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap py-3 pl-6 pr-3">
                      <div className="flex justify-end gap-3">
                        <UpdateButton
                          url={`${PATHS.joblogsEdit}/${joblog.id}`}
                        />
                        <DeleteButton
                          url={`${PATHS.joblogsDelete}/${joblog.id}`}
                        />
